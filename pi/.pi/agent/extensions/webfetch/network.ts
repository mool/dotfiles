...
) => Promise<readonly PublicDnsRecord[]>;

export interface FetchPublicWebClientOptions {
	readonly lookup?: PublicDnsLookup;
}
...
export async function fetchWithRedirects(
	...
	while (true) {
		...
		if (options.blockPrivateHosts) {
			await assertPublicUrl(currentUrl, options.signal);
		}
		if (options.signal?.aborted) {
			throw options.signal.reason instanceof Error ? options.signal.reason : new Error("Operation cancelled");
		}
...
async function assertPublicUrl(url: URL, signal?: AbortSignal): Promise<void> {
	...
	try {
		const records = await lookupWithAbort(hostname, lookup, signal);
		if (records.length === 0) {
...
export class FetchPublicWebClient implements PublicWebClient {
	...
	constructor(options: FetchPublicWebClientOptions = {}) {
		this.dnsLookup = options.lookup ?? lookup;
	}
	...
	async get(
		...
		const firstFetch = await fetchWithUserAgent(request, request.userAgent, options.signal, this.dnsLookup);
		...
		if (isCloudflareChallenge(response)) {
			await response.body?.cancel().catch(() => undefined);
			const retryFetch = await fetchWithUserAgent(request, request.fallbackUserAgent, options.signal, this.dnsLookup);
...
async function fetchWithUserAgent(
	...
	while (true) {
		...
		if (request.blockPrivateHosts) {
			const publicCheck = await checkPublicUrl(currentUrl, currentPublicUrl.value, signal, dnsLookup);
...
async function checkPublicUrl(
	...
	let records: readonly PublicDnsRecord[];
	try {
		records = await lookupWithAbort(hostname, dnsLookup, signal);
	} catch (cause: unknown) {
		if (signal?.aborted) {
			return err(classifySignalAbort(signal, cause));
		}
		return err({ _tag: "PublicWebRequestFailed", cause });
	}
	...
	if (records.length === 0) {
		return err({
			_tag: "PublicWebRequestFailed",
			cause: new Error("DNS resolution returned no addresses"),
		});
	}
	for (const record of records) {
		if (isPrivateOrLocalIp(record.address)) {
...
function lookupWithAbort(
	...
	return new Promise<readonly PublicDnsRecord[]>((resolve, reject) => {
		let settled = false;
		...
		const onAbort = () => {
			settle(() => reject(signal.reason instanceof Error ? signal.reason : new Error("Operation cancelled")));
		};
		...
		let pendingLookup: Promise<readonly PublicDnsRecord[]>;
		try {
			pendingLookup = dnsLookup(hostname, { all: true, verbatim: true });
		} catch (cause: unknown) {
			settle(() => reject(cause));
			return;
		}
